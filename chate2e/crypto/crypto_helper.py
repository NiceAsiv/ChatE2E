...
class CryptoHelper:
    ...
    def decrypt_aes_gcm(self, key: bytes, data: bytes, iv: bytes, tag: bytes) -> bytes:
        """
        使用 AES-GCM 模式进行解密。
        :param key: 原始密钥
    ...
    def ecdh(self, pub_key: x25519.X25519PublicKey, priv_key: x25519.X25519PrivateKey) -> bytes:
        """
        执行X25519的DH密钥交换。
